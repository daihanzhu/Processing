// in this enhanced version, I made my timer more visually appealling, and the colour of the
// fill changes as the mouse position changes
...
void draw() {
  ...
  for (int y = 50; y <= height - 50; y += 50) {
    for (int x = 50; x <= width - 50; x += 50) {
      ...
      //gradian
      float brightness = map(count, 1, currentTime, mouseX, mouseY);
         if (count <= currentTime) {
          fill(238, 58, brightness);
      }
      //fill the circle 
      if (countDown) {
        fill(238, 58, brightness);
      ...
      // fills the circle if less than stroke
      if (count <= currentTime) {
        fill(238, 58, brightness);
      } else {
        noFill();
